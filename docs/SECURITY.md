...
- Guards centralizados:
  - **Páginas/RSC** (`src/lib/dal.ts`): `verifySession` / `verifyRole` / `verifyOwner` — redirecionam.
  - **APIs** (`src/server/auth/api.ts`): `requireUser` / `requireRole` / `requireAdmin` / `requireOwner` — retornam `401/403`.
...
- [ ] 2FA para Owner
- [ ] Log de auditoria (ações de admin/owner)
